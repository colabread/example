...
public class EncryptUtils
{
     ...
     * @return
     */
    public static String AES128Encrypt(String clearText, String key) {
        try {
            SecretKeySpec skeySpec = new SecretKeySpec(key.getBytes(), "AES");
            Cipher cipher = Cipher.getInstance("AES/ECB/PKCS5Padding");
            cipher.init(Cipher.ENCRYPT_MODE, skeySpec);
//            String strTmp = Base64.encodeToString(cipher.doFinal(str.getBytes()), Base64.DEFAULT);
            String strTmp = Base64.encodeToString(cipher.doFinal(clearText.getBytes()), true);
            return strTmp;
        } catch (Exception e) {
            return null;
        }
...
//            String strTmp = new String(cipher.doFinal(Base64.decode(str, Base64.DEFAULT)));
            String strTmp = new String(cipher.doFinal(Base64.decode(cipherText)));
            return strTmp;
        } catch (Exception ex) {
            return null;
        }
